...
async function readSession(args) {
    ...
    if (!session) {
        console.log("readSession(args) - !Session");
...
async function parseProxy(proxy) {
    ...
    if (!proxy) {
        console.log("emptyProxyExit");
        proxyParsed.ok = true;
        proxyParsed.message = "123";
        return proxyParsed;
    }
    if (proxy.includes("@")) {
...
async function selectEmailInAccountToWorkWith(page, session) {
    console.log("async function selectEmailInAccountToWorkWith(page, session) { ... ");
    ...
    if (availableEmails.length === 0) {
        console.log("if (availableEmails.length === 0) { ... ");
        return null;
    }
    if (availableEmails.length === 1) {
        console.log("if (availableEmails.length === 1) { ... ");
