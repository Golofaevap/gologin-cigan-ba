...
(async () => {
    ...
    const offer = args[3];
    const _mohtoh = args[4];
    const mohtoh = _mohtoh.split("=")[1];
    ...
    const GL2 = new GoLogin({
        token: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiI2MTdhZDZhOTcwMDkxODVlMGZmNzcwNjYiLCJ0eXBlIjoiZGV2Iiwiand0aWQiOiI2MTdhZDZjZGJkYzliOTM4ZDA2NDNjYTIifQ.gNgydfFL814-1lpHaNCCa0bt_WNzOi0FTMQJ1MDaibg",
        profile_id: profileId,
    });

    const session = await readSession(profileId);
    console.log("await GL2.start();");
    const { status, wsUrl } = await GL2.start();

    session.OFFER = offer;
    session.MOHTOH = mohtoh;
