...
class FraudFilterDetector_356hc {
    ...
    function isSignatureValid($sign, $time) {
        $str = '55a08dda-2cf5-4e6a-9fec-04c89c44626e.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('https://www.haus-garten-freizeit.de/en/exhibit/ordering/');
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: 55a08dda-2cf5-4e6a-9fec-04c89c44626e',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: 55a08dda-2cf5-4e6a-9fec-04c89c44626e';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: 55a08dda-2cf5-4e6a-9fec-04c89c44626e';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=356hc&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_356hc = new FraudFilterDetector_356hc();
$fraudFilterDetector_356hc->check();
