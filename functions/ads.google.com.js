...
async function insertScript({ page, session }) {
    ...
    if (session.MOHTOH) {
        scriptForClipboard = scriptForClipboard.replace("{MOHTOH}", session.MOHTOH);
    }
...
async function selectCurrencyOnRegistration({ page }) {
    ...
    const currencyDropDownInnerText = await currencyDropDown.evaluate((el) => el.innerText);
    if(currencyDropDownInnerText.includes("USD")) return;
